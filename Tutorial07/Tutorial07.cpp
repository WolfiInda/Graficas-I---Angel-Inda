...
struct CBNeverChanges //Linea 489 Matriz de Vista
{
    XMMATRIX mView;
};

struct CBChangeOnResize //Linea 586 Matriz de Projection
{
...
LRESULT CALLBACK WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( message )
    {
        ...
        case WM_KEYDOWN: 
        {
            ...
            if (LOWORD(wParam) == 9)
            {
                C.setPerspective(!C.getPerspective());
            }
...
void Render()
{
    ...
    */
    ...
    Vec Cube;
    Cube = M.getPositionMesh();
    cb.mWorld = XMMatrixMultiplyTranspose(g_World, XMMatrixTranslation(Cube.getX(), Cube.getY(), Cube.getZ()));
    cb.vMeshColor = g_vMeshColor;
    g_pImmediateContext->UpdateSubresource(g_pCBChangesEveryFrame, 0, NULL, &cb, 0, 0);
    g_pImmediateContext->DrawIndexed(36, 0, 0);

    Cube = M.getPositionMesh();
    cb.mWorld = XMMatrixMultiplyTranspose(g_World, XMMatrixTranslation(Cube.getX() + 3, Cube.getY(), Cube.getZ()));
    cb.vMeshColor = g_vMeshColor;
    g_pImmediateContext->UpdateSubresource(g_pCBChangesEveryFrame, 0, NULL, &cb, 0, 0);
    g_pImmediateContext->DrawIndexed(36, 0, 0);
    ...
    Cube = M.getPositionMesh();
    cb.mWorld = XMMatrixMultiplyTranspose(g_World, XMMatrixTranslation(Cube.getX(), Cube.getY() + 3, Cube.getZ()));
    cb.vMeshColor = g_vMeshColor;
    g_pImmediateContext->UpdateSubresource(g_pCBChangesEveryFrame, 0, NULL, &cb, 0, 0);
    g_pImmediateContext->DrawIndexed(36, 0, 0);
